...
<section class="reserve-date" >
 	<form action="{{route('reservation.date.submit')}}" method="POST">
		...
		<div class="input-group">
      	<input type="text" class="form-control" name="date" id="resDate" value="09 Jan 2018" data-date-format="dd M yyyy">
...
<section class="row table-A table-row">
	<h1 class="header-1">Reservations for <span style="color:#0383ce">{{$date}}</span></h1>
	...
	@else
		<br><p class="info-message">There are No Reservations for {{$date}}</p>
	@endif
</section>
@endsection